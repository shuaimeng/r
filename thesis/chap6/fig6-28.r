dyn.load('/Library/Java/JavaVirtualMachines/jdk1.8.0_131.jdk/Contents/Home/jre/lib/server/libjvm.dylib')
library(rJava)

setwd("/Users/mengmengjiang/all datas/viscous")

library(xlsx)

# 比较18nl/min和180nl/min下的两种液体；
# 电压选择为2kv和2.2kv两种即可

k1<-read.xlsx("liquid1.xlsx",sheetName="2kv-18",header=TRUE)
k2<-read.xlsx("liquid2.xlsx",sheetName="2kv-18",header=TRUE)
k3<-read.xlsx("liquid1.xlsx",sheetName="2.2kv-18",header=TRUE)
k4<-read.xlsx("liquid2.xlsx",sheetName="2.2kv-18",header=TRUE)

# errorbar
error.bar <- function(x, y, upper, coll,lower=upper, length=0.05,...){
if(length(x) != length(y) | length(y) !=length(lower) | length(lower) != length(upper))
stop("vectors must be same length")
arrows(x,y+upper, x, y-lower,col=coll, angle=90, code=3, length=length, ...)
}

# 布局
par(mfrow = c(2,1), mar = c(2,2.4,0.8,1), oma = c(1,1,1,1))
layout(matrix(c(1,2), 2, 1,byrow = TRUE))

# yan
yan<-c("red","blue","black","green3")
pcc<-c(0,1,2,5)

# plot

plot(k1$fv,k1$deva, col=0,xlab = expression(italic(f["v"]) (Hz)),
ylab = expression(italic(d["d"])(um)), mgp=c(1.1, 0, 0),tck=0.02,
main = "", xlim = c(0,800),ylim=c(0,80))

mtext("18nl/min-d",3,line=-1,cex=1,font=2)

# lines and error

lines(k1$fv,k1$deva,col=yan[1],lwd=1.5,lty=2,type="b",pch=pcc[1],cex=0.8)
lines(k2$fv,k2$deva,col=yan[2],lwd=1.5,lty=2,type="b",pch=pcc[2],cex=0.8)
lines(k3$fv,k3$deva,col=yan[3],lwd=1.5,lty=2,type="b",pch=pcc[3],cex=0.8)
lines(k4$fv,k4$deva,col=yan[4],lwd=1.5,lty=2,type="b",pch=pcc[4],cex=0.8)

error.bar(k1$fv,k1$deva,k1$stdd/2,col=yan[1])
error.bar(k2$fv,k2$deva,k2$stdd/2,col=yan[2])
error.bar(k3$fv,k3$deva,k3$stdd/2,col=yan[3])
error.bar(k4$fv,k4$deva,k4$stdd/2,col=yan[4])

leg<-c("2kv-Liquid1","2kv-Liquid2","2.2kv-Liquid1","2.2kv-Liquid2")


legend("topright",legend=leg,col=yan,pch=pcc,lwd=1.5,lty=2,inset = .01,bty = "n",cex=0.8)

# plot 2 for 180nl/min

q1<-read.xlsx("liquid1.xlsx",sheetName="2kv-180",header=TRUE)
q2<-read.xlsx("liquid2.xlsx",sheetName="2kv-180",header=TRUE)
q3<-read.xlsx("liquid1.xlsx",sheetName="2.2kv-180",header=TRUE)
q4<-read.xlsx("liquid2.xlsx",sheetName="2.2kv-90",header=TRUE)


plot(q1$fv,q1$deva, col=0,xlab = expression(italic(f["v"]) (Hz)),
ylab = expression(italic(d["d"])(um)), mgp=c(1.1, 0, 0),tck=0.02,
main = "", xlim = c(0,800),ylim=c(20,80))

#mtext("18nl/min-d",3,line=-1,cex=1,font=2)
mtext("180nl/min-d",3,line=-1,font=2,cex=1)

# lines and error

lines(q1$fv,q1$deva,col=yan[1],lwd=1.5,lty=2,type="b",pch=pcc[1],cex=0.8)
lines(q2$fv,q2$deva,col=yan[2],lwd=1.5,lty=2,type="b",pch=pcc[2],cex=0.8)
lines(q3$fv,q3$deva,col=yan[3],lwd=1.5,lty=2,type="b",pch=pcc[3],cex=0.8)
lines(q4$fv,q4$deva,col=yan[4],lwd=1.5,lty=2,type="b",pch=pcc[4],cex=0.8)

error.bar(q1$fv,q1$deva,q1$stdd/2,col=yan[1])
error.bar(q2$fv,q2$deva,q2$stdd/2,col=yan[2])
error.bar(q3$fv,q3$deva,q3$stdd/2,col=yan[3])
error.bar(q4$fv,q4$deva,q4$stdd/2,col=yan[4])

leg<-c("2kv-Liquid1","2kv-Liquid2","2.2kv-Liquid1","2.2kv-Liquid2")

legend("topright",legend=leg,col=yan,pch=pcc,lwd=1.5,lty=2,inset = .01,bty = "n",cex=0.8)

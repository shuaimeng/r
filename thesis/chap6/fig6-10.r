...
k5<-read.xlsx("gly3.xlsx",sheetName="qd1-18",header=TRUE)
...
plot(k1$fv,k1$tfeva, col=0,xlab =expression(italic(f["v"]) (Hz)),ylab =expression(italic(f["p"]) (Hz)),mgp=c(1.1, 0, 0),tck=0.02, xlim = c(0,500),ylim=c(0,40))

mtext("tfor-1.8kv",col="black",3,line=-1.2,font=2,cex=1)
...
# Create a plot
  ...
  0,0),tck=0.02,main = "", xlim =c(0,500),ylim=c(0,0.8))

  mtext("tp-1.8kv",col="black",3,line=-1.2,font=2,cex=1)

...
k5<-read.xlsx("gly3.xlsx",sheetName="qd1-20",header=TRUE)
k6<-read.xlsx("gly3.xlsx",sheetName="qd2-20",header=TRUE)

#Layout

par(mar = c(2,2.4,0.6,1), oma = c(1,1,1,1))
...
mtext("tfor",col="black",3,line=-1.2,font=2,cex=0.9)
...
legend("topright",legend=leg,lwd=1.5,lty=2,pch=pcc,col=yan,inset= .02,bty="n",cex=0.8)

#small plot

par(fig=c(0.12,0.98,0.65,0.99),new=T)

plot(k1$fv,k1$tfeva,bty="n",col=0,xlab ="",ylab ="", mgp=c(1.1, 0, 0),tck=0.02,
main = "", xlim =c(500,3000),ylim=c(0,1),cex.axis=0.8)
...
ylab = expression(italic(t["p"]) (ms)), mgp=c(1.1,0,
0),tck=0.02,main = "", xlim =
c(0,500),ylim=c(0,0.8))

mtext("tp",col="black",3,line=-1.2,font=2,cex=0.9)

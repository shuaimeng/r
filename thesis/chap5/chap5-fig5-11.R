...
plot(n1$fv, n1$d_ra, xlab = expression(italic(f["v"](Hz))),
     ylab=expression(italic(d["d"](um))),mgp=c(1, 0, 0),tck=0.02,xlim=c(0,3400),ylim=c(0,80),col=0)

mtext("18nl/min",col="black",3,line=-1,font=2,cex=0.9)
#30g,32g,34g##
...
lines(lowess(n4$fv,n4$d_e,f=1/4,iter=3),col="blue",pch=15,lwd=1.5,lty=2,type="b",cex=0.8)
lines(lowess(n7$fv,n7$d_e,f=1/4,iter=3),col="black",pch=16,lwd=1.5,lty=2,type="b",cex=0.8)

legend("topright",c("25G","30G","32G","34G"),col=c("green3","red","blue","black"),pch=c(1,2,15,16),lwd=1.5,lty=2,bty="n",inset=.01,cex=0.85)

####54nl/min###
plot(n2$fv, n2$d_ra, xlab = expression(italic(f["v"](Hz))),
     ylab=expression(italic(d["d"](um))),mgp=c(1, 0, 0),tck=0.02,xlim=c(0,3000),ylim=c(0,80),col=0)

     mtext("54nl/min",col="black",3,line=-1,font=2,cex=0.9)
#30g,32g,34g##
...
####180nl/min###
plot(n3$fv, n3$d_ra, xlab = expression(italic(f["v"](Hz))),
     ylab=expression(italic(d["d"](um))),mgp=c(1, 0, 0),tck=0.01,xlim=c(0,3500),ylim=c(0,80),col=0)

     mtext("180nl/min",col="black",3,line=-1,font=2,cex=0.85)
#30g,32g,34g##

...
plot(n1$fv, n1$d_ra, xlab = expression(italic(f["v"](Hz))),
     ylab=expression(italic(D/d["d"])),mgp=c(1, 0, 0),tck=0.02,xlim=c(0,600),ylim=c(2,24),col=0)

mtext("Multi emission",col="black",3,line=-1,font=2,cex=0.8)
...
legend("topleft",legend=leg,col=yan,pch=pchc,bty="n",lwd=1.5,lty=2,inset=.01,cex=0.8)

##画图##
plot(n1$fv, n1$d_ra, xlab = expression(italic(f["v"](Hz))),
     ylab=expression(italic(D/d["d"])),mgp=c(1, 0, 0),tck=0.02,xlim=c(600,4000),ylim=c(2,25),col=0)

     mtext("Single emission",col="black",3,line=-1,font=2,cex=0.8)

...
mtext("tfor-18nl/min",3,line=-1,cex=0.9,font=2)
...
legend("topright",legend=leg,col=yan,pch=pcc,lwd=1.5,lty=2,inset=.1,bty="n",cex=0.8)

# plot2,about tp

plot(k1$fv,k1$tpeva, col=0,xlab = expression(italic(f["v"]) (Hz)),
ylab = expression(italic(t["p"])(ms)), mgp=c(1.1, 0, 0),tck=0.02,
main = "", xlim = c(0,800),ylim=c(0,1))
mtext("tp-18nl/min",3,line=-1,cex=0.9,font=2)
...
ylab = expression(italic(t["for"])(ms)), mgp=c(1.1, 0, 0),tck=0.02,
main = "", xlim = c(0,800),ylim=c(0,40))
mtext("tfor-180nl/min",3,line=-1,cex=0.9,font=2)
...
mtext("tp-180nl/min",3,line=-1,cex=0.9,font=2)

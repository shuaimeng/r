...
#q5<-read.xlsx("t_for.xls", sheetName = "27nl", header = TRUE)
#q5<-read.xlsx("t_for.xls", sheetName = "54nl", header = TRUE)
...
##################################画图--占空比k为0.3#######################
plot(q5$fv, q5$X0.2, xaxs = "i", col = 0, xlab = "Voltage frequency (Hz)", ylab = "Formation time t_for (ms)", main = "Formation time in Q 180nl/min", cex.lab = 1.2, cex.main = 1.5, xlim = c(0, 3550), ylim = c(0.2, 20))
...
plot(q5$fv, q5$X0.2, xaxs = "i", col = 0, xlab = "0 ~ 250 Hz", ylab = "t_for (ms)", main = "", cex.lab = 1.2, cex.main = 1.5, xlim = c(0, 250), ylim = c(0.2, 20))

#model1 = loess(q5$X0.2 ~ q5$fv, span = 0.8 , degree = 2)
lines(q5$fv, q5$X0.2,  col="#458B74", lwd=2.5, type="l", lty=3)

#model2 = loess(q5$X0.3 ~ q5$fv, span = 0.8, degree = 2)
lines(q5$fv, q5$X0.3, col="#FF00FF", lwd=2.5, type="l", lty=3)

#model3 = loess(q5$X0.4 ~ q5$fv, span = 0.8, degree = 2)
lines(q5$fv, q5$X0.4, col="#EE0000", lwd=2.5, type="l", lty=3)

#model4 = loess(q5$X0.5 ~ q5$fv, span = 0.8, degree =2)
lines(q5$fv, q5$X0.5, col="#27408B", lwd=2.5, type="l", lty=3)
...
plot(q5$fv, q5$X0.2, xaxs = "i", col = 0, xlab = "0 ~ 125 Hz", ylab = "t_for (ms)", main = "", cex.lab = 1.2, cex.main = 1.5, xlim = c(0, 125), ylim = c(0.2, 20))

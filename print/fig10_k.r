...
mtext("Sd and slope fit",3,line=0.2,font=2,cex=1.2)

points(k1$ux,k1$sdeva,col=yan[1],pch=pcc[1])
points(k2$ux,k2$sdeva,col=yan[2],pch=pcc[2])
points(k3$ux,k3$sdeva,col=yan[3],pch=pcc[3])
points(k4$ux,k4$sdeva,col=yan[4],pch=pcc[4])

z1<-lm(k1$sdeva~k1$ux)
z2<-lm(k2$sdeva~k2$ux)
z3<-lm(k3$sdeva~k3$ux)
z4<-lm(k4$sdeva~k4$ux)
...
leg1<-c("fit","fit by fv")

legend("topright",legend=leg1,col=c("red","blue"),pch=c(0,1),lwd=1.5,lty=2,inset=.02,bty="n")

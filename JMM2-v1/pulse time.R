...
par(mfrow = c(2,2), mar = c(2.2,2.4,2,2), oma = c(1,1,1,1))
mycolors <- c("green4", "black", "red", "blue")
pchc <- c(0,1,2,5)

##################################画图--占空比k为0.2#######################
plot(s2$fv, s2$X1.5, xaxs = "i", col = 0, xlab = expression(italic(f["v"]) (Hz)), 
     ylab = expression(italic(t["p"]) (ms)),mgp=c(1.3, 0, 0),tck=0.03,  main = "k = 0.2", cex.lab = 1,
     cex.main = 1.3, yaxs = "r", xlim = c(0, 1000), cex=0.7,ylim = c(0, 1))
...
legend("topright", c("1.5nl/min", "27nl/min", "54nl/min", "180nl/min"),
       col = mycolors, inset = .1,cex = 1, pch =pchc, bty = "n")

##################################画图--占空比k为0.3#######################
plot(s3$fv, s3$X1.5, xaxs = "i", col = 0, xlab = expression(italic(f["v"]) (Hz)), 
     ylab = expression(italic(t["p"]) (ms)),cex=0.7, mgp=c(1.3, 0, 0),tck=0.03, main = "k = 0.3", 
     cex.lab = 1, cex.main = 1.3, yaxs = "r", 
...
abline(0.9, -0.001, lty = 3, col = "red", lwd = 2)
text(500, 1, "0.5ms ~ 0.9ms", col ="blue", cex =1.2, font =2)
#text(20, 0.95, "(b)", col ="black", cex =1.2, font =2)
legend("topright", c("1.5nl/min", "27nl/min", "54nl/min", "180nl/min"), 
       inset = .1, col = mycolors, cex = 1, pch =pchc, bty = "n")

##################################画图--占空比k为0.4#######################
plot(s4$fv, s4$X1.5, xaxs = "i", col = 0, xlab = expression(italic(f["v"]) (Hz)), 
     ylab = expression(italic(t["p"]) (ms)),cex=0.7, mgp=c(1.3, 0, 0),tck=0.03, main = "k = 0.4", 
     cex.lab = 1, cex.main = 1.3, yaxs = "r", 
...
legend("topright", c("1.5nl/min", "27nl/min", "54nl/min", "180nl/min"), 
       inset = 0.1, col = mycolors, cex = 1, pch =pchc, bty = "n")

##################################画图--占空比k为0.5#######################
plot(s5$fv, s5$X1.5, xaxs = "i", col = 0, xlab = expression(italic(f["v"]) (Hz)), 
     ylab = expression(italic(t["p"]) (ms)),cex=0.7,mgp=c(1.3, 0, 0),tck=0.03,  main = "k = 0.5",
     cex.lab = 1, cex.main = 1.3, yaxs = "r", 

...
###画图###
###
plot(na$fv, na$he_ra, xlab = expression(paste("Qk","/",f["v"],D^"3")),
     ylab=expression(paste("d","/","D")),mgp=c(1, 0, 0),tck=0.02,
...
points(lowess(b,nb$d_eRn,f=1/4,iter=3),col="blue",pch=2,lwd=2,lty=2)
points(lowess(c,nc$d_eRn,f=1/4,iter=3),col="black",pch=15,lwd=2,lty=2)
#points(lowess(d,na360$d_e,f=1/4,iter=3),col="green3",pch=16,lwd=2,lty=2)

#a<-log(500*18/(ng$fv*60*1.9^3))
#b<-log(500*54/(nh$fv*60*1.9^3))
#c<-log(500*180/(ni$fv*60*1.9^3))
#d<-log(500*360/(nc360$fv*60*1.9^3))

#points(lowess(a,ng$d_e,f=1/4,iter=3),col="red",pch=1,lwd=2,lty=2)
#points(lowess(b,nh$d_e,f=1/4,iter=3),col="blue",pch=2,lwd=2,lty=2)
#points(lowess(c,ni$d_e,f=1/4,iter=3),col="black",pch=15,lwd=2,lty=2)
#points(lowess(d,nc360$d_e,f=1/4,iter=3),col="green3",pch=16,lwd=2,lty=2)


title(main="d/D ~ Q_e in 30G",line=0.5)

...
###画图###
###
plot(na$fv, na$he_ra, xlab = expression(paste("Qk","/",f["v"],Rn^"3")),
     ylab=expression(paste("d","/","Rn")),mgp=c(1, 0, 0),tck=0.02,
     xlim=c(-6,5),ylim=c(0,0.3),col=0)


a<-log(500*18/(na$fv*60*3.1^3))
b<-log(500*54/(nb$fv*60*3.1^3))
c<-log(500*180/(nc$fv*60*3.1^3))
d<-log(500*360/(na360$fv*60*3.1^3))

points(lowess(a,na$d_Rn,f=1/4,iter=3),col="red",pch=1,lwd=2,lty=2)
points(lowess(b,nb$d_Rn,f=1/4,iter=3),col="blue",pch=2,lwd=2,lty=2)
points(lowess(c,nc$d_Rn,f=1/4,iter=3),col="black",pch=15,lwd=2,lty=2)
#points(lowess(d,na360$d_Rn,f=1/4,iter=3),col="green3",pch=16,lwd=2,lty=2)

d<-log(500*18/(nd$fv*60*2.3^3))
e<-log(500*54/(ne$fv*60*2.3^3))
f<-log(500*180/(nf$fv*60*2.3^3))
#d<-log(500*360/(nc360$fv*60*1.9^3))

points(lowess(d,nd$d_Rn,f=1/4,iter=3),col="red",pch=1,lwd=2,lty=2)
points(lowess(e,ne$d_Rn,f=1/4,iter=3),col="blue",pch=2,lwd=2,lty=2)
points(lowess(f,nf$d_Rn,f=1/4,iter=3),col="black",pch=15,lwd=2,lty=2)

g<-log(500*18/(ng$fv*60*1.9^3))
h<-log(500*54/(nh$fv*60*1.9^3))
i<-log(500*180/(ni$fv*60*1.9^3))
#d<-log(500*360/(nc360$fv*60*1.9^3))

points(lowess(g,ng$d_Rn,f=1/4,iter=3),col="red",pch=1,lwd=2,lty=2)
points(lowess(h,nh$d_Rn,f=1/4,iter=3),col="blue",pch=2,lwd=2,lty=2)
points(lowess(i,ni$d_Rn,f=1/4,iter=3),col="black",pch=15,lwd=2,lty=2)
#points(lowess(d,nc360$d_Rn,f=1/4,iter=3),col="green3",pch=16,lwd=2,lty=2)


title(main="d/Rn ~ Q_e in 30G",line=0.5)

legend("bottom",c("18nlmin","54nlmin","180nlmin"),

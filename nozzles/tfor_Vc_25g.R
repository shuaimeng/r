...
plot(na$fv, na$d_ra, xlab = expression(f["v"](Hz)),
     ylab=expression(t["for"](ms)),mgp=c(1, 0, 0),
     tck=0.02,xlim=c(0,1000),ylim=c(0,40),col=0)

#30g
#lines(lowess(na$fv,na$tf,f=1/4,iter=3),col="red",pch=1,lwd=2,lty=2,type="b")
#lines(lowess(nb$fv,nb$tf,f=1/4,iter=3),col="blue",pch=2,lwd=2,lty=2,type="b")
#lines(lowess(nc$fv,nc$tf,f=1/4,iter=3),col="black",pch=15,lwd=2,lty=2,type="b")


#32g
#lines(lowess(nd$fv,nd$tf,f=1/4,iter=3),col="red",pch=1,lwd=2,lty=2,type="b")
#lines(lowess(ne$fv,ne$tf,f=1/4,iter=3),col="blue",pch=2,lwd=2,lty=2,type="b")
#lines(lowess(nf$fv,nf$tf,f=1/4,iter=3),col="black",pch=15,lwd=2,lty=2,type="b")

#34g
lines(lowess(nk$fv,nk$tf,f=1/4,iter=3),col="red",pch=1,lwd=2,lty=2,type="b")
lines(lowess(nl$fv,nl$tf,f=1/4,iter=3),col="blue",pch=2,lwd=2,lty=2,type="b")
lines(lowess(nd1$fv,nd1$tf,f=1/4,iter=3),col="black",pch=1,lwd=2,lty=2,type="b")
lines(lowess(nd2$fv,nd2$tf,f=1/4,iter=3),col="green3",pch=2,lwd=2,lty=2,type="b")


title(main="tfor in Vc&25G",line=0.5)

legend("topright",c("2kv-54nlmin","2kv-180nlmin",
                    "2.2kv-54nlmin","2.2kv-180nlmin"),
       pch=c(1,2,1,2),bty="n",col=c("red","blue","black","green3"),inset=.1)

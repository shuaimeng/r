...
#2###18nl-32g###
b<-log(500*18/(n7$fv*60*2.3^3))
b1<-log((500/n7$fv - n7$tf)/n7$tp)

#3#54nl-30g##
c<-log(500*54/(n5$fv*60*3.1^3))

c1<-log((500/n5$fv - n5$tf)/n5$tp)

#4#18nl-34G##
d<-log(500*18/(n10$fv*60*1.9^3))
...
#6#54nl-34G##
f<-log(500*54/(n11$fv*60*1.9^3))

f1<-log((500/n11$fv - n11$tf)/n11$tp)

#7#180nl-30G##
g<-log(500*180/(n6$fv*60*3.1^3))
g1<-log((500/n6$fv - n6$tf)/n6$tp)

#8#180nl-32G##
h<-log(500*180/(n9$fv*60*2.3^3))

...
###画图###
###
plot(na$fv, na$d_ra, xlab = expression(h["m"](um)),
     ylab=expression(f["e"](Hz)),mgp=c(1, 0, 0),tck=0.02,xlim=c(100,150),ylim=c(0,150),col=0)


#25g
#lines(lowess(nj$fv,((500/nj$fv - nj$tf)/nj$tp),f=1/4,iter=3),col="red",pch=1,lwd=2,lty=2)
#lines(lowess(nk$fv,((500/nk$fv - nk$tf)/nk$tp),f=1/4,iter=3),col="blue",pch=2,lwd=2,lty=2)
#lines(lowess(nl$fv,((500/nl$fv - nl$tf)/nl$tp),f=1/4,iter=3),col="black",pch=15,lwd=2,lty=2)
#lines(nd2$fv,nd2$d_r,col="blue",pch=16,lwd=2,lty=2)



#30g
points(na$he_r,((500/na$fv - na$tf)/na$tp),col="red",pch=1,lwd=2,lty=2)
points(nb$he_r,((500/nb$fv - nb$tf)/nb$tp),col="blue",pch=2,lwd=2,lty=2)
points(nc$he_r,((500/nc$fv - nc$tf)/nc$tp),col="black",pch=15,lwd=2,lty=2)
points(na360$he_r,((500/na360$fv - na360$tf)/na360$tp),col="green3",pch=16,lwd=2,lty=2)

#32g
#lines(lowess(nd$fv,((500/nd$fv - nd$tf)/nd$tp),f=1/4,iter=3),col="red",pch=1,lwd=2,lty=2,type="b")
#lines(lowess(ne$fv,((500/ne$fv - ne$tf)/ne$tp),f=1/4,iter=3),col="blue",pch=2,lwd=2,lty=2,type="b")
#lines(lowess(nf$fv,((500/nf$fv - nf$tf)/nf$tp),f=1/4,iter=3),col="black",pch=15,lwd=2,lty=2,type="b")


#34g
#lines(ng$fv,((500/ng$fv - ng$tf)/ng$tp),col="red",pch=1,lwd=2,lty=2,type="b")
#lines(nh$fv,((500/nh$fv - nh$tf)/nh$tp),col="blue",pch=2,lwd=2,lty=2,type="b")
#lines(ni$fv,((500/ni$fv - ni$tf)/ni$tp),col="black",pch=15,lwd=2,lty=2,type="b")
#lines(nc360$fv,((500/nc360$fv - nc360$tf)/nc360$tp),col="green3",pch=16,lwd=2,lty=2,type="b")

title(main="Fe&he in 30G",line=0.5)

legend("topright",c("18nlmin",
                    "54nlmin","180nlmin","360nlmin"),

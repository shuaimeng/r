...
plot(na$fv, na$he_raa, xlab = expression(f["v"](Hz)),
     ...
     tck=0.02,xlim=c(0,1000),ylim=c(0,60),col=0)


#25g
lines(lowess(nk$fv,(atan(nk$an)*180/pi),f=1/4,iter=4),col="red",pch=1,lwd=2,lty=2,type="b")
lines(lowess(nl$fv,(atan(nl$an)*180/pi),f=1/4,iter=3),col="blue",pch=2,lwd=2,lty=2,type="b")
lines(lowess(nd1$fv,(atan(nd1$an)*180/pi),f=1/4,iter=3),col="red",pch=15,lwd=2,lty=2,type="b")
lines(lowess(nd2$fv,(atan(nd2$an)*180/pi),f=1/4,iter=3),col="blue",pch=16,lwd=2,lty=2,type="b")
...
title(main="Angles in kv&25G",line=0.5)

legend("bottom",c("2kv-54nlmin","2kv-180nlmin","2.2kv-54nlmin","2.2kv-180nlmin"),

...
plot(na$fv, na$d_ra, xlab = expression(f["v"](Hz)),
     ylab=expression(f["e"](Hz)),mgp=c(1, 0, 0),tck=0.02,xlim=c(0,1500),ylim=c(0,100),col=0)


#25g
lines(lowess(nk$fv,((500/nk$fv - nk$tf)/nk$tp),f=1/4,iter=3),col="red",pch=1,lwd=2,lty=2,type="b")
lines(lowess(nl$fv,((500/nl$fv - nl$tf)/nl$tp),f=1/4,iter=3),col="blue",pch=2,lwd=2,lty=2,type="b")
lines(lowess(nd1$fv,((500/nd1$fv - nd1$tf)/nd1$tp),f=1/4,iter=3),col="red",pch=15,lwd=2,lty=2,type="b")
lines(lowess(nd2$fv,((500/nd2$fv - nd2$tf)/nd2$tp),f=1/4,iter=3),col="blue",pch=16,lwd=2,lty=2,type="b")
...
#lines(lowess(nc$fv,((500/nc$fv - nc$tf)/nc$tp),f=1/4,iter=3),col="blue",pch=2,lwd=2,lty=2,type="b")
...
#lines(lowess(ne$fv,((500/ne$fv - ne$tf)/ne$tp),f=1/4,iter=3),col="black",pch=15,lwd=2,lty=2,type="b")
...
#lines(lowess(nh$fv,((500/nh$fv - nh$tf)/nh$tp),f=1/4,iter=3),col="green3",pch=16,lwd=2,lty=2,type="b")
#lines(lowess(ni$fv,((500/ni$fv - ni$tf)/ni$tp),f=1/4,iter=3),col="green3",pch=16,lwd=2,lty=2,type="b")
#lines(lowess(nc360$fv,nc360$d_r,f=1/4,iter=3),col="green3",pch=16,lwd=2,lty=2,type="b")

title(main="Fe in kv&25G",line=0.5)

legend("topright",c("2kv-54nlmin",
                    "2kv-180nlmin","2.2kv-54nlmin","2.2kv-190nlmin"),
       pch=c(1,2,15,16),bty="n",col=c("red","blue","red","blue"),inset=.1)
